...
4. If GHL accepts, updates the local `live_transfers` row with the new `closing_status`. **Does NOT touch `status_change_date`** — that column records when the opp moved to Won in the opening pipeline (the date of the live transfer event) and is set exclusively during sync from `opp.lastStatusChangeAt`. If GHL rejects, the local row is NOT changed — the endpoint returns 502 and the UI shows the error.
5. On the next sync, `syncOpportunities()` re-derives `closing_status` from GHL's now-updated state and confirms it matches. `status_change_date` also stays in sync because it always comes from the opening pipeline opp, whose `lastStatusChangeAt` hasn't changed. No drift.
